...
pub fn detect(cli_path: Option<&Path>) -> Result<PathBuf, AppError> {
    detect_with(PdftoppmLocator::from_system(cli_path))
}

pub fn detect_with(locator: PdftoppmLocator<'_>) -> Result<PathBuf, AppError> {
...
fn validate_candidate(path: PathBuf) -> Result<PathBuf, AppError> {
    if is_executable_file(&path) {
        ...
        Err(AppError::PdftoppmNotExecutable { path })
    }
}
...
mod tests {
    ...
    fn path_lookup_is_used_last() {
        ...
        let detected = detect_with(locator).expect("path lookup should find candidate");
    ...
    fn create_path_candidate() -> PathBuf {
        ...
        fs::write(&file, b"test").expect("should create candidate");
    ...
    fn unique_test_dir(label: &str) -> PathBuf {
        let nonce = SystemTime::now()
            ...
            .as_nanos();
        std::env::temp_dir().join(format!("nata-pdftoppm-{label}-{nonce}"))
    }
}
